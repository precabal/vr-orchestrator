...
namespace AssemblyCSharp
{
	public class GlobalTimeLine
	{
		...
		private void LoadTextFiles()
		{
			...
			theSourceFile = new FileInfo (Application.dataPath + "/positions.txt");
			if ( theSourceFile != null && theSourceFile.Exists )
				reader = theSourceFile.OpenText();
			
			if ( reader == null )
			{
				Debug.Log("positions.txt not found or not readable");
			}
			else
			{
				string txt;
				...
				while ( ( txt = reader.ReadLine()) != null ){
					...
					_figure.AddPoint( new Vector3(Int32.Parse(result[0]), Int32.Parse(result[1]), 20.0f)  );
				}
...
//			_timeLines.Add (swarmTimeLine);
			...
			spheresTimeLine.AddEvent (new DrawFigureEvent (8.0f, _figure));
